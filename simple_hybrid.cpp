...
static bool gpu_search(faiss::Index* gpu_index, int nq_gpu, float* query_buffer, int k, faiss::Index::idx_t* I, float* D) {
	if (nq_gpu > 0) gpu_index->search(nq_gpu, query_buffer, k, D, I);
	return true;
}
...
int main(int argc, char** argv) {
	...
	if (argc != 7) {
		std::printf("Wrong usage.\n./simple <gpu_slice> <begin> <end> <step> <ncentroids> <nprobe>\n");
	}

	double gpu_slice = atof(argv[1]);
	...
	for (int nq = begin; nq <= end; nq += step) {
		...
		while (repeats <= 3) {
			...
			std::future<bool> fut = std::async(std::launch::async, gpu_search, gpu_index, nq_gpu, queries, cfg.k, I, D);
			if (nq_cpu > 0) cpu_index->search(nq_cpu, queries + nq_gpu * cfg.d, cfg.k, D + nq_gpu * cfg.k, I + nq_gpu * cfg.k);
			bool ret = fut.get();
